...
void test_uninitialized_led_driver(void)
{
    ...
    led_driver_init(pin, NULL);
    TEST_ASSERT_EQUAL_INT(LED_DRIVER_UNINITIALIZED, led_driver_set_state(LED_DRIVER_LOW));

    led_driver_mcal_t mcal = {.pin_mode = NULL, .digital_write = bsp_digital_write, .digital_read = bsp_digital_read};

    led_driver_init(pin, &mcal);
    TEST_ASSERT_EQUAL_INT(LED_DRIVER_UNINITIALIZED, led_driver_set_state(LED_DRIVER_LOW));

    led_driver_mcal_t mcal_1 = {.pin_mode = NULL, .digital_write = NULL, .digital_read = NULL};
    led_driver_init(pin, &mcal_1);
    TEST_ASSERT_EQUAL_INT(LED_DRIVER_UNINITIALIZED, led_driver_set_state(LED_DRIVER_LOW));

    led_driver_mcal_t mcal_2 = {.pin_mode = bsp_pin_mode, .digital_write = NULL, .digital_read = bsp_digital_read};
    led_driver_init(pin, &mcal_2);
    TEST_ASSERT_EQUAL_INT(LED_DRIVER_UNINITIALIZED, led_driver_set_state(LED_DRIVER_LOW));
}

void test_initialized_led_driver(void)
{
    led_driver_mcal_t interface = {
        .pin_mode = bsp_pin_mode,
        .digital_write = bsp_digital_write,
        .digital_read = bsp_digital_read};
