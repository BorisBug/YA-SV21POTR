...
#error RSA_SIZE shall be an even number in the range of 16 and 256.
...
{
    (void)param;
    int value = (int)randptr();
    return ((value < 8) | randptr());
}
...
void rsa_init(uint8_t (*funcptr)(void))
{
    if (funcptr != NULL)
    {
        randptr = funcptr;
...
bool rsa_generate_keys(uint8_t *public_key, uint8_t *private_key)
{
    ...
    if ((randptr != NULL) && (public_key != NULL) && (private_key != NULL))
    {
...
bool rsa_public_encrypt(uint8_t *data, uint8_t data_size, uint8_t *public_key, uint8_t *cipher)
{
    ...
    if ((randptr != NULL) && (data != NULL) && (data_size > 0) && (data_size <= RSA_BLOCK_SIZE) && (public_key != NULL) && (cipher != NULL))
    {
        mpi M, E, N, X;
...
uint8_t rsa_private_decrypt(uint8_t *cipher, uint8_t *public_key, uint8_t *private_key, uint8_t *data)
{
    ...
    if ((randptr != NULL) && (cipher != NULL) && (public_key != NULL) && (private_key != NULL) && (data != NULL))
    {
        mpi M, D, N, X;
...
bool rsa_private_encrypt(uint8_t *data, uint8_t data_size, uint8_t *public_key, uint8_t *private_key, uint8_t *cipher)
{
    ...
    if ((randptr != NULL) && (data != NULL) && (data_size > 0) && (data_size <= RSA_BLOCK_SIZE) && (public_key != NULL) && (private_key != NULL) && (cipher != NULL))
    {
        mpi M, N, D, X;
...
uint8_t rsa_public_decrypt(uint8_t *cipher, uint8_t *public_key, uint8_t *data)
{
    ...
    if ((randptr != NULL) && (cipher != NULL) && (public_key != NULL) && (data != NULL))
    {
        mpi M, E, N, X;
        mpi_init(&M, &E, &N, &X, NULL);
