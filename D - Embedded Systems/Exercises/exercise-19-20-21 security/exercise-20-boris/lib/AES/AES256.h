 ...
 *
 * @param funcptr A function pointer to a random number generator.
 */
void aes256_init(uint8_t (*funcptr)(void));

/**
 * @brief This function is used to nitialize/create the AES-256 key (32 bytes)
 *
 * @param key The AES-256 key(32 bytes). If key is NULL, a random AES-256 key is generated.
 * @return const uint8_t* NULL in the case of errors; otherwise a pointer to the current AES-256 key (32 bytes).
 */
 ...
 * @return uint8_t Size of the decrypted data. In the case of errors, 0 is returned;
 */
