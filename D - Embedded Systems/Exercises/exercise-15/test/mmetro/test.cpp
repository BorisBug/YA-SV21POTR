...
void test_metro_init(void)
{
    mmetro_init(NULL);
    TEST_ASSERT_NULL(mmetro_create(false, 0));

    mmetro_bsp_t mcal_0 = {.malloc = NULL, .micros = dmicros, .millis = dmillis};
    mmetro_init(&mcal_0);
    TEST_ASSERT_NULL(mmetro_create(false, 10));

    mmetro_bsp_t mcal_1 = {.malloc = NULL, .micros = NULL, .millis = NULL};
    mmetro_init(&mcal_1);
    TEST_ASSERT_NULL(mmetro_create(false, 10));

    mmetro_bsp_t mcal_2 = {.malloc = dmalloc, .micros = NULL, .millis = dmillis};
    mmetro_init(&mcal_2);
    TEST_ASSERT_NULL(mmetro_create(false, 10));

    mmetro_bsp_t mcal_3 = {.malloc = dmalloc, .micros = dmicros, .millis = NULL};
    mmetro_init(&mcal_3);
    TEST_ASSERT_NULL(mmetro_create(false, 10));

    mmetro_reset(NULL);
    TEST_ASSERT_EQUAL_UINT8(MMETRO_ERROR, mmetro_check(NULL));

    mmetro_bsp_t bsp = {.malloc = dmalloc, .micros = dmicros, .millis = dmillis};
